...
//            ,"""
//            Ogolnie jestem zadowolony, ale moj angielski nie pozwala na wyrazenie mojej opini, cena moglaby byc nizsza.
//            """
    );
    ...
    void testingSentiment() {
        ...
        Map<String, Object> reviewMap = IntStream.range(0, reviews.size())
                .boxed()
        ...
        System.out.println(chatModel.call(promptTemplate.create(reviewMap)).getResult().getOutput().getText());
    }

    @Test
    @Disabled
    void testingSentimentPart2() {
        String sentimentPrompt = """
                Determine the sentiment of the following reviews and for each return simple answer: POSITIVE or NEGATIVE.
        ...
        String response = chatModel.call(promptTemplate.create(reviewMap)).getResult().getOutput().getText();
        List<String> reviewSentiments = Arrays.stream(response.split("\n")).toList();
        assertThat(reviewSentiments).hasSameElementsAs(List.of("1. POSITIVE", "2. POSITIVE", "3. NEGATIVE", "4. NEGATIVE", "5. NEGATIVE", "6. NEGATIVE"));
    }
